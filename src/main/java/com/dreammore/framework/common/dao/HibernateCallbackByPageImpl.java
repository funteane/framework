...
//	private Log log = LogFactory.getLog(HibernateCallbackByPageImpl.class); 
	...
	public HibernateCallbackByPageImpl(String hql, Object[] param, PageBean pageObject, boolean isNativeSql) {	
		this.hql = hql;
		this.pageObject = pageObject;
		this.param = param;	
		this.isNativeSql = isNativeSql;
	...
	public Object doInHibernate(Session session) {
		...
		if(this.pageObject!=null){
			...
			String getTotolCountHql = "";		
			
			if (hql.toLowerCase().indexOf("union") >=0 || hql.toLowerCase().indexOf("group") >= 0) {
				getTotolCountHql = "select count(*) from ("+hql + ") as unionTable ";		
				jump = true;
			}
			...
			if (!jump) {
				...
				getTotolCountHql = "select count(*) "+hql;
...
//    				pageObject.setTotalRecords(((BigDecimal)(countList.get(0))).intValue());
    				pageObject.setTotalRecords(((BigInteger)(countList.get(0))).intValue());
    			}else{
